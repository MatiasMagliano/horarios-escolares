<div class="container py-3">
    <div class="d-flex flex-column flex-lg-row gap-3 justify-content-between align-items-lg-center mb-3">
        <div>
            <h3 class="mb-1">Administración de permisos</h3>
            <p class="text-muted mb-0">Permisos por rol e institución, usando equipos de Spatie.</p>
        </div>
        ...
        <div class="d-flex flex-column flex-sm-row gap-2">
            ...
            <select wire:model.live="institucionId" class="form-select">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-light d-flex flex-column flex-md-row gap-2 justify-content-between align-items-md-center">
            <div>
                ...
                <p class="text-muted small mb-0">Seleccioná qué acciones puede realizar este rol.</p>
            </div>

            <div class="d-flex gap-2">
                <button type="button" wire:click="aplicarPreset" class="btn btn-outline-secondary btn-sm">
                    Aplicar preset
                </button>
                ...
                <table class="table table-sm table-bordered align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="text-center" style="width: 12%;">Habilitado</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($modules as $moduleKey => $module)
                            @foreach ($module['permissions'] as $actionKey => $label)
                                <tr wire:key="permission-{{ $moduleKey }}-{{ $actionKey }}">
                                    @if ($loop->first)
